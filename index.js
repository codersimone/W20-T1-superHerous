...
//output of info to the browser (showSlides function)
function showSlides(n) {
    //writing to the variable n index in a JSON string
    const heroe = superHeroesBase[n];
    //writing content output to the browser to a variable
    const content = `<div class='superHeroe'>
    <img src="${heroe.superHeroePic}" style="width: 600px">
    <h2>${heroe.superHeroeCharacter}</h2>
    <div><b>Вселенная:</b> ${heroe.comicsUniverse}</div>
    <div><b>Альтер эго:</b> ${heroe.alterEgo}</div>
    <div><b>Род деятельности:</b> ${heroe.typeOfActivity}</div>
    <div><b>Друзья:</b> ${heroe.friends}</div>
    <div><b>Суперсилы:</b> ${heroe.superPowers}</div>
...
//slide gallery 
//creating a variable with the current index of the JSON string value
let currentSlideIndex = 0;

//previous button 
function previousSlide() {
    //the current index of the JSON string value minus another index = rewinding the gallery one slide back and showing this slide (-=)
    currentSlideIndex -= 1;
    //we show minus one more index from the end of the array length if the current index is less than 0
    if (currentSlideIndex < 0) {
        currentSlideIndex = superHeroesBase.length - 1;
    }
    //showSlides function call 
    showSlides(currentSlideIndex);
}

//next button 
function nextSlide() {
    //the current index of the JSON string value plus another index = rewind the gallery one slide ahead and show this slide (+=)
    currentSlideIndex += 1;
    //return to the first (0) index when the loop reaches the end of the length of the array of objects (>=)
    if (currentSlideIndex >= superHeroesBase.length) {
        currentSlideIndex = 0
    }
    //showSlides function call 
    showSlides(currentSlideIndex);

    //the second solution for scrolling slides
    // if (currentSlideIndex >= superHeroesBase.length - 1) {
    //     currentSlideIndex = 0
    // }
    // else {
    //     currentSlideIndex += 1;
