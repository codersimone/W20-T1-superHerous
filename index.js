...
let arraySuperHeroesJson = `[{
    ...
    "moreDetailed": "как и Росомаха из Людей Икс, Дэдпул был подвергнут опытам по программе «Оружие Икс». Ученые попытались исцелить его рак, привив его клеткам способность к регенерации. Как и всегда в комиксах, что-то пошло не так, и Дэдпул остался изуродованным и психически нестабильным.
...
document.addEventListener("DOMContentLoaded", function(event) {
    let superHeroes = JSON.parse(arraySuperHeroesJson);
    console.log(superHeroes);
    let superHeroesContent = "";
    for (let superHeroe of superHeroes) {
        superHeroesContent += `<div class='superHeroe'>
        <h2>${superHeroe.superHeroeCharacter}</h2>
        <div>Вселенная: ${superHeroe.comicsUniverse}</div>
        </div>`;
    }
    document.getElementById("SuperheroeDescription").innerHTML = superHeroesContent;
});



//output of info to the browser

//arraySuperheroesJson[]